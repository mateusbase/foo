  ...
  return (
    <div className="mt-3 flex flex-col items-center gap-6 md:mt-10 lg:flex-row lg:justify-center">
      {projects.map((project) => (
        <div
          key={project.id}
          className="flex h-[105px] w-full flex-row items-center gap-6 rounded-[20px] border border-lightGray py-8 pl-7 md:h-[201px] md:w-[420px] md:py-12"
        >
          <Image
            src={project.image}
            alt={project.title}
          ...
          <div>
            ...
            <p className="text-lg md:text-2xl">{project.text}</p>
          </div>
        </div>
      ))}
    </div>

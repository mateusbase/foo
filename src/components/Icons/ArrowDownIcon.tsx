...
type ArrowDownIconProps = React.SVGProps<SVGSVGElement> & {
  size?: number;
  color?: string; // Controlar diretamente a cor
  className?: string;
};

const ArrowDownIcon = ({
  size = 1,
  color = "text-primary", // Aplicando uma cor padrão
  className = "",
  ...props
  ...
  <svg
    ...
    viewBox="0 0 24 14"
    fill="none"
    className={`${color} ${className}`} // Aplica o color do Tailwind e as classes adicionais
    {...props}
  >
    <path
      fill="currentColor" // O currentColor herda a cor aplicada no svg
